...
cli, serr = socket.connect(host, port)

if serr then
	print(serr)
	os.exit(0)
end
print("Conectado a: "..cli:getpeername() )
	cli:settimeout(1,"t")
	local mymsg = sendmsg(num)
	send, serr = cli:send(mymsg)
	print("Envio: ",mymsg, send, serr)
	local ok = false
	local msg = ""
	while ok ~= true do
		local m, serr = cli:receive(1)
		print("Primero", m, serr)
		if m then
			if m ~= string.char(6) then
				msg = msg .. m
				if m == parityBit(msg) then
					ok = true
				end
			end
		end
	...
	while ok ~= true do
		local m, serr = cli:receive(1)
		print("segundo", m, serr, msg,string.len(msg))
		if m then
			if m ~= string.char(6) then
				if m == parityBit(msg) then
					print(msg, parityBit(msg), string.len(msg))
					ok = true
				end
				msg = msg .. m
			end
	...
	while ok ~= true do
		local m, serr = cli:receive(1)
		print("tercero", m, serr, msg,string.len(msg))
		if m then
			if m ~= string.char(6) then
				if m == parityBit(msg) then
					print(msg, parityBit(msg), string.len(msg))
