...
--
		...
		for _, cmd in ipairs(cmdsec) do
			t.count[cmd] = 0
			t.fail[cmd] = 0
			t.status[strF[cmd]] = { msg="Desconocido", hora=socket.gettime(), detalle="Inicio programa controlador", errorpercent = 0, count=0, fail=0}
			t.prev[strF[cmd]] = { msg="Desconocido", hora=socket.gettime(), detalle="Inicio programa controlador", errorpercent = 0, count=0, fail=0}
		end
...
function setStatus(m,msg,detalle)
	...
	m.status[strF[m.f]].errorpercent = string.format("%05.2f",m.status[strF[m.f]].fail/m.status[strF[m.f]].count*100)
end
...
function setdate(m)
	...
	if tiempo - m.timelastset > 60 or hora.min == 0 then
		m.f = "1"
...
print("RECIBE desde server ",str, serr)
			if serr == "closed" then
				params.srvskt:close()
				params.srvskt = nil
				connectInfoSrv(params)
