	...
	if self.scripts then
		for i, t in pairs(self.scripts) do
			if type(t) == "table" then
				...
				if nt.charset then strscript = strscript ..'CHARSET="'..nt.charset..'" ' end
				if nt.type then strscript = strscript ..'TYPE="'..nt.type..'" ' end
				if nt.src then strscript = strscript ..'SRC="'..nt.src..'" ' end
--				if nt.languaje then strscript = strscript ..'LANGUAJE="'..nt.languaje..'" ' end -- deprecated not standar
				if nt.defer then strscript = strscript ..'DEFER="'..nt.defer..'" ' end
				strscript = strscript..">"
...
htmlContentClass = {}
htmlContentClass_mt = {__index = htmlContentClass}

function htmlContentClass.new()
	local self = {}
	setmetatable(self,htmlContentClass_mt)
	return self 
end

function htmlContentClass:add(str)
	if str then
		self[#self+1] = str
		if type(str) == "table" then
			if str.id then 
...
function htmlContentClass:text()
	local str = ""
	for i, t in ipairs(self) do
...
function htmlContentClass:print()
	print(self:text())
end

htmlTagClass = {}
htmlTagClass_mt = {__index = htmlTagClass} 

function htmlTagClass.new(tag, id)
	local self = {}
	self.tag = tag
	self.content = htmlContentClass.new()
	self.attr = {}
	if id then
		if type(id) == "table" then
			self.attr = id
			if self.attr.id then
				self.id = id.id
				self.attr.id = nil
			else
				self.id = "unknow"
	...
	setmetatable(self,htmlTagClass_mt)
	return self 
end

function htmlTagClass:setAttribute(attr,value)
...
--[[
function htmlTagClass:setContent(value)
	if value then
...
function htmlTagClass:text()
	local str = "<"..self.tag
	if self.id and self.id ~= "" then
		str = str..string.format(" ID=\"%s\"",self.id)
	end
	for k, v in pairs(self.attr) do
		str = str..string.format(' %s="%s"',k,v)
	end
	str = str..">\n"
	for i, s in ipairs(self.content) do
...
htmlbodyClass = {}
htmlbodyClass_mt = {__index = htmlbodyClass} 
function htmlbodyClass.new(tag, id)
	local self = {}
	self.attr = {}
	setmetatable(self,htmlbodyClass_mt)
	return self 
end

function htmlbodyClass:add(attr,value)
	if attr and value then
		self.attr[attr] = value
	end
end

function htmlbodyClass:text()
	local str = "<BODY"
	for k, v in pairs(self.attr) do
		str = str..string.format(' %s="%s"',k,v)
	end
...
function htmlpageClass:buildBody ()
end
